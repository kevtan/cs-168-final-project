...
This is the documentation of the ILSVRC2016 Competitions Development Kit.

There are five datasets for this year's competitions:
* Object detection dataset (DET) -- same as ILSVRC2015
* Object classification/localization dataset (CLS-LOC) -- same as ILSVRC2012
* Object detection from video dataset (VID) -- same as ILSVRC2015
* Places2 scene classification dataset -- new in ILSVRC2016
* Places2 scene segmentation dataset -- new in ILSVRC2016
...
Table of contents:
  ...
  5. Places2 scene classification and scene segmentation competition details
...
Key difference between ILSVRC2015 and ILSVRC2016 data:
...
between the three sets. We will partially refresh the test dataset.
...
DET statistics:
  ...
  Test:
    ...
    - 51294 images fully annotated with the 200 object categories.
...
the challenge at
    ...
    http://www.image-net.org/challenges/LSVRC/2016
...
The main evaluation metric is the same as the object detection (DET) task,
where we treat each frame in a snippet as an individual image and assign
predicted bounding box to ground truth box in each frame. Following DET,
objects which are not annotated will be penalized, as will be duplicate
detections (two annotations for the same object instance).

Besides the main metric, we have an auxiliary metric where we assign predicted
tracklet to ground truth tracklet in a snippet. The score of a tracklet is the
mean score of all the detections within it. A predicted tracklet T of class X is
considered to have properly localized a ground truth tracklet Tgt of class X if

  IOU(T,Tgt) = (T intersection Tgt) / (T union Tgt) >= thr

where we use 0.5 IOU threshold for per frame bounding box matching and use
multiple IOU threshold (e.g. 0.25, 0.5, 0.75) for tracklet matching. The final
mAP is an average over mAPs computed with different tracklet IOU threshold.

Duplicate tracklets are considered as false detections. All tracklets which do
not properly localize a ground truth tracklet of the right class according to
the threshold are considered as false detections as well.
...
with one line per predicted object. It looks as follows:
  ...
  <frame_index> <ILSVRC2015_VID_ID> <track_id> <confidence> <xmin> <ymin> <xmax> <ymax>

frame_index corresponds to the frame index from val.txt or test.txt,
depending on which set of images is being annotated.

An example submission file for the main metric based on the validation data is
provided (Note: track_id is -1 for all detections.):

  evaluation/demo.val.pred.vid.txt

An example submission file for the new auxiliary metric is provided:

  evaluation/demo.val.track.vid.txt

*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
4.5 VID evaluation routines
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*

The Matlab routine for evaluating a submission using the main metric is
...
blacklist_file:
NOTE: you can specify a cache filename and the ground truth data will be automatically cached to save loading time in the future.
We already provide the cached ground truth file: ../data/ILSVRC2015_vid_validation_ground_truth.mat
...
The Matlab routine for evaluating a submission using the new auxiliary metric is

   evaluation/eval_vid_tracking.m
...
DETECTION FROM VIDEO TASK WITH TRACKING
pred_file: demo.val.track.vid.txt
meta_file: ../data/meta_vid.mat
eval_file: ../../ImageSets/VID/val.txt
blacklist_file:
NOTE: you can specify a cache filename and the ground truth data will be automatically cached to save loading time in the future.
We already provide the cached ground truth file: ../data/ILSVRC2015_vid_validation_track_ground_truth.mat
Please enter the path to the Validation bounding box annotations directory: ../../Annotations/VID/val
-------------
Category        AP
airplane        0.465
...
Please refer to http://places2.csail.mit.edu/index.html for more details.
